...
export const APHeader: React.FC<{ title: string; subtitle: string; svg?: string }> = ({ title, subtitle, svg }) => (
    <div className="flex justify-center w-[70%] items-center flex-col space-y-2">
        <div className="text-5xl font-bold text-center">{title}</div>
        <div className="text-2xl text-center">{subtitle}</div>
        {svg && <img src={svg} alt="SVG Icon" style={{ marginTop: '50px' }} />}
    </div>
);


export const APInstruction: React.FC<{ instruction: string }> = ({ instruction }) => (
...
export const ActivityPage: React.FC<ActivityPageProps> = ({ onBack, onContinue, onExit, bgColor, buttonColor, numColor, num, title, subtitle, instruction, svg }) => (
    <APContainer
        bgColor={bgColor}
        ...
        onExit={onExit}
    >
        <APHeader title={title} subtitle={subtitle} svg={svg} />
